...
class AdminBackupController extends AdminController
{
    ...
    {
        $backups = AdminBackup::query()->with('creator:id,name,email')->latest()->paginate(25);
        $stats = [
    ...
    public function store(Request $request, AdminBackupService $service): RedirectResponse
    {
        $validated = $request->validate(['type' => ['required', 'in:database,full']]);

        try {
            $backup = $service->create($validated['type'], $request->user()->id);
            $request->user()->notify(new AdminSystemNotification(
                'Backup completed',
    ...
    {
        abort_unless($backup->status === 'completed' && Storage::disk($backup->disk)->exists($backup->file_path), 404);
        return Storage::disk($backup->disk)->download($backup->file_path, $backup->name);
    }
    ...
    {
        $validated = $request->validate(['days' => ['required', 'integer', 'in:7,14,30,60,90']]);
        $oldBackups = AdminBackup::query()->where('created_at', '<', now()->subDays((int) $validated['days']))->get();
        foreach ($oldBackups as $backup) {
            Storage::disk($backup->disk)->delete($backup->file_path);
            $backup->delete();
        }
        return back()->with('success', $oldBackups->count() . ' old backup(s) deleted.');
    }
