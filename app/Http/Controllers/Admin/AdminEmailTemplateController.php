...
class AdminEmailTemplateController extends AdminController
{
    ...
    public function update(
        ...
        $emailTemplate->update([
            ...
            'admin_id' => (int) $admin->getAuthIdentifier(),
            'updated_by' => null,
        ]);
    ...
    public function preview(
        ...
        return view('emails.dynamic-template', [
            'renderedSubject' => $service->replace(
                $emailTemplate->subject,
                $variables,
                false
            ),
            'renderedBody' => $service->sanitizeHtml(
                $service->replace(
                    $emailTemplate->body,
                    $variables,
                    true
    ...
    public function sendTest(
        ...
        $subject = $service->replace(
            $emailTemplate->subject,
            $variables,
            false
        );

        $body = $service->sanitizeHtml(
            $service->replace(
                $emailTemplate->body,
                $variables,
                true
        ...
        Mail::send(
            ...
            function ($message) use ($validated, $subject): void {
                $message
                    ->to($validated['email'])
    ...
    private function sampleVariables(
        EmailTemplate $template
    ): array {
        $defaults = [
            'customer_name' => 'Alex Customer',
            'order_number' => 'AO-10025',
            'order_total' => '149.99',
            'currency' => 'USD',
            'payment_status' => 'Paid',
            'order_url' => url('/dashboard'),
            'admin_order_url' => url('/admin/orders/10025'),
            'alert_type' => 'Low stock',
            'item_name' => 'Classic Arizona Hoodie — Black / Medium',
            'current_stock' => '3',
            'threshold' => '5',
            'inventory_url' => url('/admin/inventory-alerts'),
