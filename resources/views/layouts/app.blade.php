...
<head>
    ...
    <title>@yield('title', $title ?? 'IdeoStream')</title>
...
@if(request()->routeIs('blogs-page', 'blog-show', 'categories-page', 'category-show'))
...
@if(request()->routeIs('services-page.index', 'services-show.show'))
    <script src="{{ asset('asset/js/services.js') }}"></script>
@endif

@if(request()->routeIs('projects-page.index', 'projects-show.show'))
    <script src="{{ asset('asset/js/project-main.js') }}"></script>
@endif

@if(request()->routeIs('privacy-policy-page', 'terms-and-conditions-page'))
